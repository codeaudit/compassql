...
import {isDimension, isMeasure, EncodingQuery, isFieldQuery, isValueQuery, isAutoCountQuery} from './query/encoding';
...
export class SpecQueryModel {
  ...
  constructor(spec: SpecQuery, wildcardIndex: WildcardIndex, schema: Schema, opt: QueryConfig, wildcardAssignment: Dict<any>) {
    ...
    this._channelFieldCount = spec.encodings.reduce((m, encQ) => {
      ...
      if (isAutoCountQuery(encQ) && !isWildcard(encQ.channel) && encQ.autoCount !== false) {
        m[encQ.channel + ''] = 1;
      }
      return m;
    }, {} as Dict<number>);
  ...
  private _encoding(): Encoding {
    ...
    for (const encQ of this._spec.encodings) {
      ...
      if (isFieldQuery(encQ) && isAutoCountQuery(encQ) && encQ.autoCount === true) {
